...
articles = [
    {
        ...
        <div style="text-align: justify; text-indent: 30px; width: 50%;">
            ...
            обработкой данных и парсинг в целом идет быстрее. Второй слой Core предназначен для длительного хранения 
            данных. Тут основная цель уменьшить вес базы. Происходит это за счет создания словарей. Допустим у нас есть 
            ...
            <br>
            <img src='../file/dwh_1.png' style='width: 100%; height: auto;'><br>
            <br>
            ...
            запускаться ночью и работать пока не получит все данные. Добавил в базу таблицу с календарем. После получения 
            токена пришлось переписать основной код парсера. Теперь я сначала получал id вакансий тем же способом что и 
            раньше. А потом запрашивал подробную информацию по этим вакансиям. Так как для каждой вакансии указано 
            ...
            запросы в секунду. В результате десятки тысяч вакансий обрабатываются почти весь день. Написал процедуры для 
            укладывания данных в Core. Там обошлось почти без проблем. Разве что даже в моей маленькой базе пришлось 
            думать об оптимизации в некоторых местах.  <br>
            <br>
            ...
            только то что я хочу видеть. Чтобы построить дашборд настроил yandex data lens и подключил его к базе. <br> 
            <br>
            <img src='file/dwh_2.png' style='width: 100%; height: auto;'><br>
            <br>
            Провел тестирование и так вышло что некоторые регионы я не указал при парсинге, а значит данные по ним не 
            попали в базу. Поэтому написал функцию для добавления новых регионов в существующую базу, провел повторное 
            тестирование. Теперь база готова к постоянному сбору данных.<br>
        </div>
        """
    },
    {
        ...
        "full_text": """
        <div style="text-align: justify; text-indent: 30px; width: 50%;">
            Это история о том, как я начал свой путь в разработке и какие трудности встретил на своем пути.
        </div>
        """
    }
]
