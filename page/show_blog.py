...
articles = [
    ...
    },
    {
        ...
        "description": "Расширение домашней инфраструктуры для удобного деплоя проектов.",
        "full_text":
        """<div style="text-align: justify; text-indent: 30px; width: 50%;">
            ...
            <img src='https://raw.githubusercontent.com/kulakov544/site_vizitka/main/file/infrastructure_1.png' style='width: 100%; height: auto;'><br>
            <br>
            ...
            <br>
            <img src='https://raw.githubusercontent.com/kulakov544/site_vizitka/main/file/infrastructure.png' style='width: 100%; height: auto;'><br>
            <br>
    ...
    },
    {
        ...
        "description": "Установка PowerBI Report Server на домашний сервер. Настройка доступа к отчетам хранящимся в базе данных.",
        "full_text": """
        <div style="text-align: justify; text-indent: 30px; width: 50%;"> Наверное лучшим средством 
        для визуализации данных является PowerBI. В нем можно строить дашборды любой сложности. Он предоставляет 
        множество инструментов для работы с данными. Подключается к большинству источников данных. Хранит загруженные 
        данные в формате значительно ускоряющем быстродействие. А простейший дашборд можно построить практически без 
        обучения. Вот только купить его в России с недавнего времени больше нельзя. Осталась только бесплатная версия 
        для личного пользования. Недавно я развернул PowerBI Report Server на своем домашнем сервере и могу 
        рассказать как это было. <br> 
        Всё начинается с установки MS SQL Сервера<br> 
        <img src='https://raw.githubusercontent.com/kulakov544/site_vizitka/main/file/bi_1.png' style='width: 100%; height: auto;'><br> 
        <br> 
        Подойдет любая версия кроме Express. В этой базе будут храниться готовые отчеты к которым будет давать доступ 
        PowerBI Report Server. Для полноценной работы MS Сервер нужно установить почти в полной комплектации. <br>
        ...
        сохранить созданный в ней дашборд в базу данных. После этого если привязать подключение базы данных к 
        доменному имени можно будет иметь доступ к своим отчетам из любой точки мира. Естественно где есть устройство 
        с интернетом.<br> 
        <img src='https://raw.githubusercontent.com/kulakov544/site_vizitka/main/file/bi_3.png' style='width: 100%; height: auto;'><br> 
        <br> Не обошлось и без проблем. Связка из MS SQL Сервер + 
        PowerBI Report Server + Power BI Desktop оказалась довольно капризная. Части PowerBI должны быть одного 
        месяца иначе не будут нормально работа. Драйвер для подключения к PosgreSQL должен быть только одной 
        определенной версии Npgsql-4.0.10. И с настройкой доступа могут возникнуть проблемы. У меня ушло много 
        времени на настройку Nginx для доступа к отчетам по доменному имени.<br>
        <br>
        За то если пройти все препятствия получим очень мощный инструмент для визуализации данных на собственном сервере.
        </div>"""
    },
]
